...
def main():
    ...
    server.bind(('', port_no))
    server.listen(2)
    server.settimeout(1.0)  # Set a timeout on the accept call
    ...
    try:
        while True:
            try:
                ...
                # Submit handling of client to the thread pool
                executor.submit(handle_client, client_socket, client_addr)
            except socket.timeout:
                # Timeout to allow periodic checking for KeyboardInterrupt
                continue
...
if __name__ == "__main__":
    try:
        main()  # Start the main server logic
    except KeyboardInterrupt:
